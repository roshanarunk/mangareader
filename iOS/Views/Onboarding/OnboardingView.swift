    ...
    var body: some View {
        VStack(alignment: .center) {
            ...
            VStack(alignment: .leading) {
                Text("Welcome to Mangareader")
                    .font(.title3)
                    .fontWeight(.semibold)
                Text("Mangareader is a comic reader which allows you to read your favorite comics from a variety of sources")
                    .font(.subheadline)
                    .fontWeight(.light)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            Text("A plethora of options.")
                ...
                .fontWeight(.semibold)
            Group {
                Text("Mangareader allows you to read comics from an OPDS Server, Archived Files (CBR, CBZ, RAR, ZIP) and from external runners using custom scripts written in Javascript.")
            }
...
struct OnboardingAddListsTabView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Adding Lists")
                ...
                .fontWeight(.semibold)
            Group {
                Text("You can add lists to Mangareader by simply visiting the website of the List and tapping the *Add To Mangareader* button. Your saved lists will then be available under **Saved Lists** in \(Image(systemName: "ellipsis.circle")).\n\nTo help you get started tap the button below to add the Community List containing popular Sources & Trackers directly supported by us.")
            }
            .font(.subheadline.weight(.light))
    ...
    func saveCommunity() {
        Task {
            do {
                try await DSK.shared.saveRunnerList(at: "https://community.Mangareader.app")
...
struct OnboardingAddRunnersView: View {
    ...
    private let COMMUNITY_LIST_URL = "https://community.Mangareader.app"
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Adding Runners")
                .font(.title3)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("All Done!")
                ...
                .fontWeight(.semibold)
            Group {
                Text("Great! Now you're ready to begin reading from external sources. To view your saved lists or installed runners navigate to the \(Image(systemName: "ellipsis.circle")) tab.\n\nFor further questions regarding *runners*, visit our [website](https://Mangareader.app) and feel free to drop your questions over on our [**Discord Server**](https://discord.gg/8wmkXsT6h5), where users can also share their created lists.\n\nWe hope this was a tad bit helpful and Happy Reading!")
            }
            .font(.subheadline.weight(.light))
